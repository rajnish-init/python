...
# for i in range(1, n+1):  # i is the loop variable, which takes on each number in the sequence generated by range(1, n+1).
#     sum += i # sum += i is shorthand for sum = sum + i. It adds the current value of i to the existing value of sum and stores the result back in sum.
